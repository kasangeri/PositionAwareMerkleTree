...
if __name__ == "__main__":
	# a) Create the new class of MerkleTree
	#i=0
	Tree = MerkleTree(1)
	fileblock = []
	n  = int(raw_input("Enter number files to be uploaded: "))
	for i in range(0,n):
		...
		with open(filename,"rb") as f:
			...
			print readable_hash
    			fileblock.append([readable_hash,1]) #Hash of the file and the number of children


	# b) Give list of fileblock
	#fileblock = [['a',1],['b',1],['c',1],['d',1],['e',1]]
	# c) pass on the fileblock list 
	Tree.listoffileblock = fileblock

	# d) Create the Merkle Tree fileblock
	Tree.create_tree(1)

	# e) Retrieve the fileblock 
	past_fileblock = Tree.Get_past_transacion()
